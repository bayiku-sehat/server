...
class BayiController {
  ...
  static async showDetail(req, res, next) {
    console.log('baby by id', req.params)
    try {
  ...
  static async add(req, res, next) {
    try {
      ...
      if (
        params.tinggi < whoTinggiBadan.sd_1 &&
        params.tinggi > whoTinggiBadan.sd_n1
      ) {
        params.status_tinggi = 0
      } else if (
        params.tinggi >= whoTinggiBadan.sd_1 &&
        params.tinggi < whoTinggiBadan.sd_2
      ) {
        params.status_tinggi = 1
      } else if (
        params.tinggi >= whoTinggiBadan.sd_2 &&
        params.tinggi < whoTinggiBadan.sd_3
      ) {
        ...
        params.tinggi <= whoTinggiBadan.sd_n1 &&
        params.tinggi > whoTinggiBadan.sd_n2
      ) {
        params.status_tinggi = -1
      } else if (
        params.tinggi <= whoTinggiBadan.sd_n2 &&
        params.tinggi >= whoTinggiBadan.sd_n3
      ) {
        params.status_tinggi = -2
      ...
      if (
        params.berat_badan < whoBeratBadan.sd_1 &&
        params.berat_badan > whoBeratBadan.sd_n1
      ) {
        params.status_berat_badan = 0
      } else if (
        params.berat_badan >= whoBeratBadan.sd_1 &&
        params.berat_badan < whoBeratBadan.sd_2
      ) {
        params.status_berat_badan = 1
      } else if (
        params.berat_badan >= whoBeratBadan.sd_2 &&
        params.berat_badan < whoBeratBadan.sd_3
      ) {
        ...
        params.berat_badan <= whoBeratBadan.sd_n1 &&
        params.berat_badan > whoBeratBadan.sd_n2
      ) {
        params.status_berat_badan = -1
      } else if (
        params.berat_badan <= whoBeratBadan.sd_n2 &&
        params.berat_badan >= whoBeratBadan.sd_n3
      ) {
        params.status_berat_badan = -2
      ...
      if (
        params.lingkar_kepala < whoLingkarKepala.sd_1 &&
        params.lingkar_kepala > whoLingkarKepala.sd_n1
      ) {
        params.status_lingkar_kepala = 0
      } else if (
        params.lingkar_kepala >= whoLingkarKepala.sd_1 &&
        params.lingkar_kepala < whoLingkarKepala.sd_2
      ) {
        params.status_lingkar_kepala = 1
      } else if (
        params.lingkar_kepala >= whoLingkarKepala.sd_2 &&
        params.lingkar_kepala < whoLingkarKepala.sd_3
      ) {
        ...
        params.lingkar_kepala <= whoLingkarKepala.sd_n1 &&
        params.lingkar_kepala > whoLingkarKepala.sd_n2
      ) {
        params.status_lingkar_kepala = -1
      } else if (
        params.lingkar_kepala <= whoLingkarKepala.sd_n2 &&
        params.lingkar_kepala >= whoLingkarKepala.sd_n3
      ) {
        params.status_lingkar_kepala = -2
